...
scoped_refptr<EphemeralStoragePartition> EphemeralStoragePartition::GetOrCreate(
    StoragePartitionImpl* default_storage_partition,
    BrowserContext* browser_context,
    std::string storage_domain) {
  ...
  auto path = base::FilePath(std::string("ephemeral") + storage_domain);
  EphemeralStorageParitionMapKey key =
      std::make_pair(browser_context, storage_domain);
  return base::MakeRefCounted<EphemeralStoragePartition>(
      key, default_storage_partition, path);
}
...
leveldb_proto::ProtoDatabaseProvider*
EphemeralStoragePartition::GetProtoDatabaseProviderForTesting() {
  return non_ephemeral_storage_partition_->GetProtoDatabaseProviderForTesting();
}
...
void EphemeralStoragePartition::InitNetworkContext() {
  ...
  GetContentClient()->browser()->ConfigureNetworkContextParams(
      browser_context, /* in_memory = */ true, relative_partition_path_,
      context_params.get(), cert_verifier_creation_params.get());
  devtools_instrumentation::ApplyNetworkContextParamsOverrides(
      browser_context, context_params.get());
